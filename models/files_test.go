...
func testFileToOneLanguageUsingLanguage(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadLanguage(ctx, tx, false, (*[]*File)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testFileToOneSetOpLanguageUsingLanguage(t *testing.T) {
	var err error
...
var (
	fileDBTypes = map[string]string{`ID`: `bigint`, `Hash`: `character varying`, `LanguageID`: `integer`, `Content`: `text`, `CreatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
